...
class QueueDbHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertRequest(url: String, payload: String): Long {
    ...
    fun getPendingCount(): Int {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT COUNT(*) FROM $TABLE_REQUESTS WHERE $COL_STATUS = 'PENDING'", null)
        cursor.use {
            if (it.moveToFirst()) {
                return it.getInt(0)
            }
