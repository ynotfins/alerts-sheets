 ...
 */
export async function getFeatureFlag(flagName: keyof typeof DEFAULT_FLAGS): Promise<boolean | number> {
  try {
    const db = getDb();
 ...
 */
export async function setFeatureFlag(flagName: keyof typeof DEFAULT_FLAGS, value: boolean | number): Promise<void> {
  try {
    const db = getDb();
    await db.collection(CONFIG_COLLECTION).doc('featureFlags').set(
      { [flagName]: value },
      { merge: true }
    );
 ...
 */
export async function getAllFeatureFlags(): Promise<Record<string, boolean | number>> {
  try {
    const db = getDb();
    const doc = await db.collection(CONFIG_COLLECTION).doc('featureFlags').get();
    ...
    return doc.data() as Record<string, boolean | number>;
  } catch (error) {
