...
function doPost(e) {
  ...
  try {
    ...
    const cleanDetail = incomingDetail.replace(/<C> BNN/gi, "").replace(/BNNDESK.*$/gi, "").trim();
    ...
    if (foundRow !== -1) {
      ...
      const uniqueCodes = Array.from(allCodes).filter(c => 
          c.toLowerCase() !== "bnn" && 
      ...
      if (uniqueCodes.length > 0) {
        sheet.getRange(foundRow, 11, 1, uniqueCodes.length).setValues([uniqueCodes]);
      }
    ...
    return ContentService.createTextOutput(JSON.stringify({ "result": "success", "id": incidentId })).setMimeType(ContentService.MimeType.JSON);
